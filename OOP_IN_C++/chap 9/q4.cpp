...
class disk: public publication, public sales{
    char d;
    string dtype;
    string enum_tp(char x){
        switch (x){
            case 'c':{
                return static_cast<string>("CD");break;
            }
            case 'd':{
                return static_cast<string>("DVD");break;
            } //  warning: control reaches end of non-void function [-Wreturn-type] 
        }    // Code works but error still appears
    };
    public:
        void getdata(){
            publication::getdata();
            cout << "Disk Type(CD(c) or DVD(d)): "; cin >> d;
            dtype = enum_tp(d);
            sales::getdata();
        }
        void putdata(){
             publication::putdata(); cout << "Disk Type: " << dtype << endl; sales::putdata();
            }
};
